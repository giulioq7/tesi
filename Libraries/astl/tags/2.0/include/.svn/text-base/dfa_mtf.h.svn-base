...
// Descrition:	
//   ASTL 2.0 Determinisic Finite Automaton Class Template DFA_mtf
//   Representation by arrays with move-to-front method                 

#ifndef ASTL_DFA_MTF_H
#define ASTL_DFA_MTF_H
...
class DFA_mtf 
  : public DFA_base<Sigma_, Tag_, 
		    list<pair<typename Sigma_::char_type, unsigned int> > >
{
protected:
  typedef list<pair<typename Sigma_::char_type, unsigned int> > 
  transition_container;
	
public:
  typedef DFA_base<Sigma_, Tag_, list<pair<typename Sigma_::char_type, 
					   unsigned int> > > super;
  ...
  public:
    ...
    size_type size() const {
      return (container->size()); // warn: std::list::size() allowed linear time
     }
    ...
    const_iterator upper_bound(const key_type& x) const { 
      ...
      return result == end() ? result : ++result;
    }
  ...
  void del_trans(state_type s, const char_type &a)
  {
    Q[s]->edges().erase(find_if(Q[s]->edges().begin(), Q[s]->edges().end(), 
				compose1(bind2nd(typename DFA_mtf<Sigma_,Tag_>::edges_type::key_compare(), a), 
